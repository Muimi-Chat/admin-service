...
@csrf_exempt
def disable_admin_totp(request):
    ...
    if not _can_perform_action(account, AccessAttribute.REVOKE_ADMIN_2FA):
    ...
    try:
        ...
        if target_user.totp_enabled is False:
        ...
        ServiceLog.objects.create(
            content=f"User {account.username} ({account.id}) failed to set disable user TOTP :: {traceback.format_exc()}",
    ...
    try:
        ...
        email_content = f"Dear {target_user.username},\n\nYour account's TOTP have been disabled by {account.username}."
        email_header = "Admin Account TOTP Disabled"
        ...
        ServiceLog.objects.create(
            severity=LogSeverity.ERROR,
            content=f'Failed to notify {target_user.username} ({target_user_id}) TOTP Disabled :: {traceback.format_exc()}',
        )

    ServiceLog.objects.create(
        severity=LogSeverity.VERBOSE,
        content=f'User {account.username} ({account.id}) disabled TOTP for admin {target_user.username} ({target_user_id})',
    )
